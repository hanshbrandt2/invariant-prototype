...
export function AccountMenu() {
  ...
  return (
    <div className="relative px-3 pt-3">
      ...
      >
        <span className="grid place-items-center h-7 w-7 shrink-0 bg-clay text-paper font-mono text-ui">
          {initial}
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-ui text-ink truncate">My Invariant</span>
          <span className="block font-mono text-meta text-faint truncate">{addr}</span>
        </span>
        ...
        <>
          ...
          <div className="absolute left-3 right-3 top-[calc(100%-0.25rem)] z-20 mt-1 border border-ink bg-paper">
            <div className="px-3.5 py-3 border-b border-hairline">
              <p className="text-ui text-ink truncate">{addr}</p>
              <div className="mt-1 flex items-center gap-2">
                <span className="font-mono text-meta uppercase tracking-[0.12em] border border-hairline-2 text-muted px-1.5 py-0.5">
                  Free
                </span>
                <span className="font-mono text-meta text-faint">plan</span>
              </div>
            </div>

            <div className="px-3.5 py-3 border-b border-hairline flex items-center justify-between">
              <span className="eyebrow">credits</span>
              <span className={`font-mono text-h3 tabular-nums ${balance < 10 ? "text-clay" : "text-ink"}`}>
                {balance.toFixed(1)}
              </span>
            </div>
            ...
            <Link
              ...
              className="w-full text-left px-3.5 py-2.5 text-ui text-ink flex items-center justify-between hover:bg-paper-2 transition-colors"
            >
              Settings
            ...
            <button
              ...
              className="w-full text-left px-3.5 py-2.5 text-ui text-ink border-t border-hairline hover:bg-paper-2 transition-colors"
            >
              Sign out
            </button>
          </div>
