    ...
    <div className="p-6 md:p-8 max-w-[760px]">
      {concept && (
        <div className="border border-hairline bg-paper p-6">
          ...
          <p className="mt-3 font-serif text-h2 leading-[1.45] text-ink">
            <span className="italic">{focusLabel}</span> is a {focusKind}
            {op && (
              <>
                {" "}
                — built by <span className="font-mono text-body text-clay">{op}</span>
              </>
            )}
            .
          </p>
          <p className="mt-4 text-body leading-[1.7] text-ink-2">{concept.what}</p>
          <p className="mt-3 text-body leading-[1.7] text-muted">{concept.why}</p>
        </div>
      )}
          ...
          <ol className="border border-hairline bg-paper divide-y divide-hairline">
            {pieces.map((p) => (
              <li key={p.kind} className="flex gap-4 px-4 py-3.5">
                <span className="font-mono text-meta uppercase tracking-[0.12em] text-clay w-20 shrink-0 pt-0.5">
                  {p.kind}
                </span>
                <span className="text-body leading-[1.6] text-ink-2">{p.what}</span>
              </li>
            ))}
          </ol>
        </div>
