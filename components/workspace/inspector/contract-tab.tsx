...
function HashChip({ label, value }: { label: string; value: string }) {
  ...
  return (
    ...
    >
      <span className="font-mono text-[0.6rem] uppercase tracking-[0.1em] text-muted shrink-0">{label}</span>
      <span className="flex items-center gap-2 min-w-0">
        <span className="font-mono text-[0.78rem] text-ink truncate">{value}</span>
        <span className={`font-mono text-[0.62rem] shrink-0 ${copied ? "text-[#3B6D11]" : "text-faint group-hover:text-clay"}`}>
          {copied ? "copied ✓" : "copy"}
        </span>
      </span>
    </button>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-1.5">
            {node.policyRefs.map((p) => (
              <span key={p} className="font-mono text-[0.7rem] border border-clay text-clay rounded-full px-2.5 py-1">
                {p.split(":")[1] ?? p}
              </span>
            ))}
          </div>
