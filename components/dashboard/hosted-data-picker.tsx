      ...
      {datasets.map((d) => (
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[0.9rem] text-ink">{d.name}</span>
            <span className="block font-mono text-[0.7rem] text-faint truncate">{d.id} · {d.schema}</span>
          </span>
          <span className="font-mono text-[0.72rem] text-muted tabular-nums shrink-0">{fmt(d.rows)} rows</span>
